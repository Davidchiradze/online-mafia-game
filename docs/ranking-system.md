...
Two parts: a **faction-calibrated base** (expected-score ELO, K = 40) plus a
...
Rounded to integers, the **base** payouts are:

| Faction  | E     | **Win** | **Loss** | EV/game for an average player |
| -------- | ----- | ------- | -------- | ----------------------------- |
| Mafia    | 0.387 | **+24** | **−15**  | +0.08                         |
| Citizens | 0.327 | **+27** | **−13**  | +0.08                         |
| Yakuza   | 0.286 | **+28** | **−11**  | +0.16                         |
| No contest | —   | 0       | 0        | 0                             |
...
Worked examples (Citizens base +27 / −13):

- You are `1000`, table average `1140` → `b = +7`. Win **+34**, loss **−6**.
- You are `1400`, table average `1150` → `b = −8` (clamped from −12.5).
  Win **+19**, loss **−21**.
- You are `1050`, table average `1050` → `b = 0`. Base numbers apply as-is.

Why these numbers are safe:

- `1 point per 20 ELO` is the **linear approximation of true Elo at K = 40** —
  not an invented knob. The hard cap of ±8 (reached at ±160 table difference)
  bounds how much any lobby can swing a result.
- Because the cap (8) is **below the smallest base numbers** (win +24,
  loss −11), a win always pays **at least +16** and a loss always costs **at
  least −3** — a win can never become ≤ 0 and a loss can never turn positive,
  no matter how lopsided the table. Maximum possible swing: win +36, loss −23.
...
- **Win ≈ 2× loss** at a balanced table — compensates for the ~33.5% average
  win rate.
- The tiny positive drift (+0.08…+0.16/game ≈ +10 rating per 100 games) is a
...
- Pacing sanity check: max single-game gain is +36 (base +28 + table cap +8)
  and the narrowest bracket is 150 wide, so a player can never skip a level in
  one game. A solidly above-average player (~40% personal win rate) gains
  ~+2.6/game at balanced tables — Level 10 is a long-term aspiration, as on
  FACEIT (and the symmetric table adjustment slows climbing further once a
  player out-rates their usual tables).
...
- Only the faction win rates (E) recalibrate. The table-adjustment constants
  (`divisor: 20`, `cap: 8`) are structural — they derive from K, not from
  data — and only change if K changes.
...
| # | Piece | Where | Notes |
| - | ----- | ----- | ----- |
| 1 | `RATING_CONFIG: Record<gameType, { start, floor, K, deltas, tableAdjustment: { divisor: 20, cap: 8 } }>` — one entry per **rated** game type (today: `japanese_mafia` only) | `convex/lib/constants.ts` | Server-only — clients never compute deltas (they read stored ones). A game type absent from the record is unrated. |
