  ...
  if (isLocal && !alwaysVisible) {
    return (
      <button
        ...
        className={`${BADGE_BASE} cursor-pointer transition-all hover:brightness-110 active:scale-95 ${getRoleColorClass(playerRole)}`}
      >
        {label}
      </button>
    );
