 ...
 * structured `WinMethod` snapshot of the winning faction, `"no_contest"`, or
 * `null` to continue. `decideWinner` is the faction-only convenience that
 ...
 *
 * The shared vocabulary (`WinContext`, `WinMethod`, `Outcome`) and the generic
 * `winMethodLabel` presentation helper stay in `convex/lib/winConditions.ts` —
 * they are cross-variant. Only these Japanese decision tables live here.
 */

import { MAFIA_TEAM_ROLES } from "../../lib/constants";
import type { Outcome, Role, WinContext, WinMethod } from "../core/types";

type Winner = WinMethod["faction"];
